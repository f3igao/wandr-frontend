...
export default class Chatroom extends Component {
	...
	sendMesssage = event => {
		fetch(`http://localhost:3000/chatrooms/${this.props.chatroom.id}/message`, {
			method: 'POST',
			headers: {
	...
	render() {
		let messageComponents = this.props.chatroom.messages.map(message => {
			return (
				<p>
					...
					<button onClick={event => this.deleteMessage(message.id)}>
						DELETE
					</button>
				</p>
			);
		...
		return (
			<div>
				...
				{messageComponents}
				<button onClick={this.props.leaveChatRoom}>Go Back</button>
			</div>
		);
	}
}
